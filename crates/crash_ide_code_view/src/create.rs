    ...
    for (entity, code_view_style, content, mut style) in query.iter_mut() {
        ...
        let line_count_container = commands.spawn(NodeBundle {
            ...
            for (index, _) in content.lines.iter().enumerate() {
                build_line_count(parent, code_view_style, index + 1, scale);
            }
        }).id();
        ...
        let line_content_container = commands.spawn((NodeBundle {
            ...
            for line in content.lines.iter() {
                build_line_parent(parent, code_view_style, line, scale);
            }
        }).id();
...
pub(crate) fn build_line_command(commands: &mut Commands, code_view_style: &CodeViewStyle, line: &CodeViewContentLine, scale: f32) -> Entity {
    commands.spawn(create_line_bundle(code_view_style, scale)).with_children(|parent| {
        spawn_lines(parent, code_view_style, line);
    }).id()
}

pub(crate) fn build_line_parent(parent: &mut ChildBuilder, code_view_style: &CodeViewStyle, line: &CodeViewContentLine, scale: f32) {
    parent.spawn(create_line_bundle(code_view_style, scale)).with_children(|parent| {
        spawn_lines(parent, code_view_style, line);
    });
}

fn create_line_bundle(code_view_style: &CodeViewStyle, scale: f32) -> impl Bundle {
    (
        NodeBundle {
            style: Style {
                ...
                height: Val::Px(code_view_style.font_size * scale + 2.0),
...
pub(crate) fn build_line_count(parent: &mut ChildBuilder, code_view_style: &CodeViewStyle, line: usize, scale: f32) {
    parent.spawn(NodeBundle {
        ...
        parent.spawn(NodeBundle {
            style: Style {
                flex_direction: FlexDirection::Row,
                align_items: AlignItems::Center,
                width: Val::Px(code_view_style.font_size * scale * 1.5),
                height: Val::Px(code_view_style.font_size * scale + 2.0),
