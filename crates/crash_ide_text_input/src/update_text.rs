...
use bevy::prelude::*;
use crate::{TextInputLineContainer, TextInputStyle, CodeViewToken, TextInputContent, TextInputCursorPosition, TextInputContentLine};
use crate::create::build_line_command;
...
impl<'w, 's> UpdateText<'w, 's> {
    pub fn insert_text(&mut self, content: &mut TextInputContent, cursor_pos: &mut TextInputCursorPosition, lines: &TextInputLineContainer, str: &str) {
        ...
        let texts_in_line = self.children_query.get(
            self.get_line_container.get_line(lines, cursor_pos.cursor_pos.y as usize),
        ).unwrap();
    ...
    pub fn insert_new_line(&mut self, content: &mut TextInputContent, cursor_pos: &mut TextInputCursorPosition, lines: &TextInputLineContainer, code_view_style: &TextInputStyle) {
        let Some((token_index, insert_index, token)) =
            content.get_line_mut(cursor_pos.cursor_pos.y as usize).unwrap()
                .get_token_mut(cursor_pos.cursor_pos.x as usize) else {
            return;
        ...
        content.lines.insert(cursor_pos.cursor_pos.y as usize, TextInputContentLine {
    ...
    pub fn remove_text(&mut self, content: &mut TextInputContent, cursor_pos: &mut TextInputCursorPosition, lines: &TextInputLineContainer, backspace: bool) {
        ...
        if let Some((from, to)) = line_to_line {
            ...
            let from_container = self.get_line_container.get_line(lines, from);
            let from_texts = self.children_query.get(from_container).unwrap();
            let to_container = self.get_line_container.get_line(lines, to);
